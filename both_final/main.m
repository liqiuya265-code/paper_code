...
% 添加障碍物（位于飞行轨迹50%处，确保无避障时导弹穿过障碍物）
obs.add_spherical_obstacle([-3000, -4000, 3100], 500);  % 阻挡 M1 路径
obs.add_cylindrical_obstacle([-5000, -2900, 0], 500, [0, 0, 1]);  % 阻挡 M2 路径（垂直圆柱）
obs.add_spherical_obstacle([-4100, -2400, 2800], 500);  % 阻挡 M3 路径
obs.add_spherical_obstacle([-3200, -3800, 2900], 500);  % 阻挡 M4 路径
        ...
        if j==M
            ...
            if i==1
                ...
                [Ay_obs, Az_obs,last_psi_i_obs{i}] = compute_control_from_observer(t(i), z_observer, a_now, a_base, ...
                    Vm', N, M, T, sigma_max, alpha, beta, p, q, m, miu, v, n, obs, omega_env_i, n_env, lambda_info, x,zeros(M, M));
            else
                [Ay_obs, Az_obs,last_psi_i_obs{i}] = compute_control_from_observer(t(i), z_observer, a_now, a_base, ...
                    Vm', N, M, T, sigma_max, alpha, beta, p, q, m, miu, v, n, obs, omega_env_i, n_env, lambda_info, x,last_psi_i_obs{i-1});
            end
