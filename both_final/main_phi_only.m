        ...
        if j==M
            ...
            if i==1
                ...
                [Ay_obs, Az_obs,last_psi_i_obs{i}] = compute_control_from_observer(t(i), z_observer, a_now, a_base, ...
                    Vm', N, M, T, sigma_max, alpha, beta, p, q, m, miu, v, n, obs, omega_env_i, n_env, lambda_info, x,zeros(M, M), 'phi');
            else
                [Ay_obs, Az_obs,last_psi_i_obs{i}] = compute_control_from_observer(t(i), z_observer, a_now, a_base, ...
                    Vm', N, M, T, sigma_max, alpha, beta, p, q, m, miu, v, n, obs, omega_env_i, n_env, lambda_info, x,last_psi_i_obs{i-1}, 'phi');
            end
...
% 保存控制量数据用于控制量对比
save('control_effort_phi_only.mat', 'Ay', 'Az', 't');
...
title('Cumulative Control Effort ($\phi$ only: $\omega_{2i}=\phi_i$)', 'FontSize', 14, 'FontName', 'Times New Roman', 'Interpreter', 'latex');
